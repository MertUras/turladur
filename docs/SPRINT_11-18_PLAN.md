...
### Definition of Done
- [x] `pnpm dev` ile hem web (3000) hem api (4000) ayağa kalkıyor
- [x] `pnpm nx run-many -t build` hatasız tamamlanıyor
...
| 12.2 | Prisma schema taşıma (mevcut → multi-schema) | P0 | Tüm tablolar schema ile ayrılmış |
| 12.3 | Migration: mevcut tabloları yeni schema'lara taşı | P0 | `catalog.*`, `identity.*`, `booking.*` |
| 12.4 | `core/cache/` — Redis module + cache service | P0 | Redis bağlantısı, get/set/del çalışıyor |
| 12.5 | `core/auth/` — JWT strategy + guards + decorators | P0 | `@CurrentUser`, `@Roles`, `@Public` |
| 12.6 | `core/storage/` — MinIO/S3 presigned URL service | P1 | Upload/download presigned URL üretimi |
| 12.7 | `core/mail/` — Resend + Mailhog (dev) | P1 | Email gönderimi çalışıyor |
| 12.8 | `core/queue/` — BullMQ setup + email processor | P1 | Background email job çalışıyor |
| 12.9 | `core/logger/` — Pino structured logging | P1 | JSON log çıktısı |
| 12.10 | `core/health/` — Health check endpoint | P2 | `/api/v1/health` → DB + Redis status |
| 12.11 | Global exception filter | P0 | `{ success, data, error }` format |
| 12.12 | Global validation pipe (whitelist + transform) | P0 | Otomatik DTO validation |

### Definition of Done
- [ ] `POST /api/v1/auth/login` → JWT döner
- [ ] Redis'e veri yazıp okunabiliyor
- [ ] Presigned URL ile MinIO'ya dosya yüklenebiliyor
- [ ] Email kuyruğa atılıp Mailhog'da görünüyor
- [ ] Hatalı request → standard error response dönüyor
...
### Definition of Done
- [ ] `POST /api/v1/identity/register` → kullanıcı oluşuyor
- [ ] `POST /api/v1/identity/login` → JWT dönüyor
- [ ] `GET /api/v1/catalog/tours/search?q=kapadokya` → sonuç dönüyor (cached)
- [ ] `POST /api/v1/catalog/tours` (Partner) → tur oluşturuluyor
- [ ] Swagger'da tüm endpoint'ler dokümante
