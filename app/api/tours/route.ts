...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Veri doğrulama
    console.log('departureCity tipi:', typeof departureCity, 'değeri:', departureCity);
    console.log('destinations tipi:', typeof destinations, 'değeri:', destinations);
    ...
    const cleanDestinations = destinations
      .filter(dest => dest && typeof dest === 'object' && dest.city && typeof dest.city === 'string' && dest.city.trim())
      .map(dest => ({
        city: dest.city.trim(),
    ...
    if (Array.isArray(departureCity)) {
      departureCityString = departureCity.filter(city => city && city.trim()).join(', ');
    ...
    // Önce turu oluştur
      ...
      for (const date of tourDates) {
        ...
        if (!date.startDate || !date.endDate) {
          console.error('Tur tarihi için başlangıç veya bitiş tarihi eksik:', date);
          continue;
        }
        ...
        console.log('Tur tarihi oluşturuluyor:', JSON.stringify(tourDateData, null, 2));
        ...
        if (date.ageRanges && Array.isArray(date.ageRanges) && date.ageRanges.length > 0) {
          ...
          console.log('Yaş aralığı verileri:', JSON.stringify(ageRangeData, null, 2));
    ...
    // Yolcu alma noktalarını ekle
      ...
      const pickupPointData = pickupPoints
        .filter((point: any) => point && typeof point === 'object' && point.city && point.location && point.time)
      ...
      console.log('Yolcu alma noktası verileri:', JSON.stringify(pickupPointData, null, 2));
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('Invalid value provided')) {
        errorMessage = 'Geçersiz veri formatı. Lütfen tüm alanları kontrol ediniz.';
      } else if (error.message.includes('Unique constraint')) {
        errorMessage = 'Bu tur adı zaten kullanılıyor. Lütfen farklı bir ad seçiniz.';
      } else if (error.message.includes('Foreign key constraint')) {
        errorMessage = 'Tur operatörü bilgisi hatalı. Lütfen tekrar giriş yapınız.';
      } else if (error.message.includes('Invalid date')) {
        errorMessage = 'Tarih formatı hatalı. Lütfen tarihleri kontrol ediniz.';
      } else if (error.message.includes('Invalid number')) {
        errorMessage = 'Sayısal değer formatı hatalı. Lütfen fiyat ve kontenjan bilgilerini kontrol ediniz.';
      } else if (error.message.includes('Required field')) {
        errorMessage = 'Zorunlu alanlar eksik. Lütfen tüm gerekli alanları doldurunuz.';
      }
    }
