...
export const PHONE_COUNTRIES: PhoneCountry[] = [
  {
    ...
    validationMessage: 'Geçerli bir Türkiye cep telefonu girin (5XX XXX XX XX).',
  },
  {
    code: 'US',
    dial: '+1',
  ...
  },
  {
    ...
    formatGroups: [4, 3, 3],
    validationMessage: 'Geçerli bir Birleşik Krallık telefon numarası girin (10 hane).',
  },
  {
    code: 'DE',
    dial: '+49',
    ...
    formatGroups: [3, 7, 1],
    validationMessage: 'Geçerli bir Almanya telefon numarası girin (10-11 hane).',
  },
  {
    code: 'FR',
    dial: '+33',
  ...
  },
  {
    ...
    formatGroups: [2, 3, 4],
    validationMessage: 'Geçerli bir Suudi Arabistan telefon numarası girin (9 hane).',
  },
  {
    code: 'AE',
    dial: '+971',
  ...
  },
  {
    ...
    validationMessage: 'Geçerli bir Azerbaycan telefon numarası girin (9 hane).',
  },
  {
    code: 'GR',
    dial: '+30',
    ...
    formatGroups: [3, 3, 4],
    validationMessage: 'Geçerli bir Yunanistan telefon numarası girin (10 hane).',
  },
];

const PHONE_RULES_BY_DIAL = new Map(PHONE_COUNTRIES.map((country) => [country.dial, country]));
...
export function formatLocalPhoneDisplay(dial: string, localNumber: string): string {
...
export function clampLocalPhoneDigits(dial: string, localNumber: string): string {
  const rule = getPhoneCountry(dial);
  return localNumber.replace(/\D/g, '').slice(0, rule.maxDigits);
}

export function isValidLocalPhone(localNumber: string, countryDial = '+90'): boolean {
  const digits = localNumber.replace(/\D/g, '');
  if (!digits) return false;

  const rule = getPhoneCountry(countryDial);
  if (digits.length < rule.minDigits || digits.length > rule.maxDigits) return false;
  if (rule.pattern && !rule.pattern.test(digits)) return false;
  return true;
}

export function isValidFullPhone(countryDial: string, localNumber: string): boolean {
