...
export default function RegisterPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === 'password') {
      ...
      if (value.length >= 8) {
        ...
        messages.push('8+ karakter');
      }
      if (/[A-Z]/.test(value)) {
        strength += 1;
      } else {
        messages.push('Büyük harf');
      }
      if (/[a-z]/.test(value)) {
        strength += 1;
      } else {
        messages.push('Küçük harf');
      }
      if (/[0-9]/.test(value)) {
        strength += 1;
      } else {
      ...
      if (/[!@#$%^&*(),.?":{}|<>-]/.test(value)) {
        ...
        messages.push('Özel karakter');
      }

      setPasswordStrength(strength);
      setPasswordMessage(
        messages.length > 0 ? `Eksik: ${messages.join(', ')}` : 'Güçlü',
      );
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (passwordStrength < 4) {
      setError(
        'Şifreniz yeterince güçlü değil. Lütfen kriterleri kontrol edin.',
      );
      return;
    }
    ...
    try {
      ...
      if (result?.error) {
        setError(
          'Kayıt başarılı ancak otomatik giriş yapılamadı. Lütfen manuel giriş yapın.',
        );
      } else if (result?.ok) {
  ...
  const baseInputClass =
    'block w-full rounded-lg border text-neutral-900 placeholder-neutral-400 focus:outline-none focus:ring-1 focus:ring-offset-0 text-sm transition duration-200 ease-in-out shadow-sm focus:bg-white';
  ...
  const successBorderClass =
    'border-emerald-500 focus:border-emerald-500 focus:ring-emerald-300';
  const inputIconClass =
    'absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none';
    ...
    <div className="h-screen flex bg-neutral-50 pt-16 overflow-hidden">
      <div className="hidden lg:block lg:w-1/2 relative h-[calc(100vh-4rem)]">
        <Image
          ...
          priority
        />
        <div className="absolute inset-0 bg-gradient-to-br from-neutral-900/85 via-neutral-900/80 to-neutral-950/85 z-10"></div>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center z-20 p-8 text-center">
          <div className="max-w-sm">
            ...
            <p className="text-base text-neutral-200/90 font-light">
          ...
          <form className="space-y-3" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              {formData.password && (
                <div className="mt-2 space-y-1">
                  ...
                  <p
                    className={`text-xs font-medium ${passwordStrength < 3 ? 'text-rose-600' : passwordStrength < 5 ? 'text-amber-600' : 'text-emerald-600'}`}
                  >
                    {passwordMessage}
                  </p>
                </div>
            ...
            <div className="flex items-start pt-1">
              <div className="flex items-center h-4 mt-0.5">
                <input
                  ...
                  onChange={handleChange}
                  className="h-3 w-3 rounded border-neutral-300 text-neutral-950 focus:ring-neutral-950 focus:ring-offset-0"
                />
              </div>
              <div className="ml-2 text-xs">
                <label
                  htmlFor="termsAccepted"
                  className="text-neutral-600 leading-snug"
                >
                  <Link
                    href="/terms"
                    target="_blank"
            ...
            <div className="pt-1">
              <button
                type="submit"
                disabled={loading}
                className="group relative w-full flex items-center justify-center py-2 px-4 border border-transparent rounded-lg text-sm font-semibold text-white bg-neutral-950 hover:bg-neutral-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-neutral-950 transition-all duration-200 ease-in-out shadow-sm disabled:opacity-60 disabled:cursor-not-allowed transform active:scale-[0.98] min-h-[36px]"
              >
                {loading ? (
                  <svg
