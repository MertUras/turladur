...
import { getOperatorDisplayName } from '@/app/lib/operator';
import type { RouteWithStats, TourWithOperator } from '@/lib/routes';
import {
  MapPinIcon,
  CalendarIcon,
  ClockIcon,
...
export default function RouteDetailClient({ routeId }: PageProps) {
  ...
  const fetchRoute = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/routes/${routeId}${query ? `?${query}` : ''}`);
      if (res.status === 404) {
        setNotFound(true);
        setData(null);
        return;
      ...
      <div className="bg-white shadow-md relative -mt-12 max-w-5xl mx-auto rounded-xl overflow-hidden">
        <div className="grid grid-cols-2 md:grid-cols-4">
          ...
          <div className="p-6 flex flex-col items-center text-center">
            ...
            <div className="font-semibold text-gray-900">{route.tourCount} tur</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-50 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-4">Bu Rotadaki Turlar</h2>
          ...
          {categoryEntries.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-2xl shadow-sm">
              <p className="text-gray-600 mb-4">
                Bu rota için henüz tur bulunmuyor veya filtrelerinize uygun tur yok.
              </p>
              <Link href={`/routes/${routeId}`} className="text-blue-600 hover:text-blue-800 font-medium">
                Filtreleri temizle
              </Link>
            </div>
          ) : (
                    ...
                    {tours.map((tour) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="p-6">
                            <p className="text-sm text-gray-500 mb-2">{operatorName}</p>
