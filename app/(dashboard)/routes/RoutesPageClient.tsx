...
export default function RoutesPageClient() {
  ...
  const fetchRoutes = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/routes${query ? `?${query}` : ''}`);
    ...
    <div className="bg-white">
      ...
      <section id="popular-routes" className="py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {loading ? (
            <div className="text-center py-16 text-gray-500">Rotalar yükleniyor...</div>
          ) : routes.length === 0 ? (
        ...
        <section className="py-16 bg-gradient-to-r from-blue-600 to-blue-800 text-white">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { number: String(stats.routeCount), label: 'Aktif Rota', icon: MapIcon },
                { number: String(stats.operatorCount), label: 'Tur Operatörü', icon: UserGroupIcon },
                { number: String(stats.tourCount), label: 'Tur Seçeneği', icon: HeartIcon },
                {
                  number: stats.avgRating !== null ? stats.avgRating.toFixed(1) : '—',
                  label: 'Ortalama Puan',
                  icon: StarIcon,
                },
              ].map((stat, index) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2">
              <div className="p-8 md:p-12 lg:p-16 flex flex-col justify-center">
                ...
                <p className="text-lg text-gray-600 mb-8">
                  Türkiye&apos;nin en popüler rotalarını, gizli kalmış destinasyonlarını ve
                  seyahat ipuçlarını içeren kapsamlı rehberimizi hemen indirin.
                </p>
