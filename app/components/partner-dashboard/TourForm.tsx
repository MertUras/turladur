...
import { PhotoIcon, XMarkIcon, PlusIcon, ChevronDownIcon, TrashIcon } from '@heroicons/react/24/outline';
...
const TOUR_OPTIONS = {
  includes: [
    'Konaklama', 'Kahvaltı', 'Öğle Yemeği', 'Akşam Yemeği', 'Rehberlik Hizmeti',
    'Ulaşım', 'Müze Giriş Ücretleri', 'Seyahat Sigortası', 'Transfer Hizmeti',
    'Wifi', 'Ara Öğün İkramları', 'Fotoğraf Çekimi'
  ],
  excludes: [
    'Alkollü İçecekler', 'Ekstra Aktiviteler', 'Kişisel Harcamalar',
    'Öğle Yemeği', 'Akşam Yemeği', 'Müze Giriş Ücretleri', 'Bahşişler',
    'Vize Ücreti', 'Yurtdışı Çıkış Harcı'
  ],
  features: [
    'Ücretsiz İptal', 'Hemen Onay', 'Çocuk İndirimi', 'Erken Rezervasyon İndirimi',
    'Tam Pansiyon', 'Yarım Pansiyon', 'Her Şey Dahil', 'Rehberli Tur',
    'Özel Araç', 'Lüks Konaklama', 'Aile Dostu', 'Engelli Dostu',
    'Evcil Hayvan Dostu', 'Fotoğraf Turu', 'Doğa Yürüyüşü', 'Deniz Aktiviteleri',
    'Kültür Turu', 'Macera Aktiviteleri'
  ]
} as const;
...
export default function TourForm({ 
  ...
  const [formData, setFormData] = useState<TourFormData>(() => {
    if (initialData) {
      ...
      return {
        ...
        departureCity: Array.isArray(initialData.departureCity) ? initialData.departureCity : (initialData.departureCity ? [initialData.departureCity] : ['']), // Dizi kontrolü
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'nights') {
    ...
    } else if (name === 'startDate' || name === 'endDate' || name === 'price' || name === 'maxParticipants') {
      ...
      setFormData(prev => {
        ...
        if (updatedTourDates.length > 0) {
          updatedTourDates[0] = {
            ...updatedTourDates[0],
            startDate: name === 'startDate' ? value : updatedTourDates[0].startDate,
            endDate: name === 'endDate' ? value : updatedTourDates[0].endDate,
            price: name === 'price' ? value : updatedTourDates[0].price,
            maxParticipants: name === 'maxParticipants' ? value : updatedTourDates[0].maxParticipants,
            minParticipants: name === 'maxParticipants' ? Math.floor(Number(value) / 2).toString() : updatedTourDates[0].minParticipants
          };
        }
        return {
          ...prev,
  ...
  const handleDateFieldChange = (field: 'startDate' | 'endDate', value: string) => {
    if (field === 'startDate') {
  ...
  const validateBasicForm = async (): Promise<boolean> => {
    ...
    else if (formData.title.length < 5) newErrors.title = 'Tur adı en az 5 karakter olmalı';
    else if (formData.title.length > 100) newErrors.title = 'Tur adı en fazla 100 karakter olmalı';
    
    if (!formData.description?.trim()) newErrors.description = 'Açıklama gerekli';
    else if (formData.description.length < 50) newErrors.description = 'Açıklama en az 50 karakter olmalı';
    
    if (!formData.price?.trim()) newErrors.price = 'Fiyat gerekli';
    else if (parseFloat(formData.price) <= 0) newErrors.price = 'Geçerli bir fiyat giriniz';
    ...
    if (!formData.departureCity || formData.departureCity.length === 0) {
      ...
      const emptyCities = formData.departureCity.filter(city => !city?.trim());
      if (emptyCities.length > 0) {
        newErrors.departureCity = 'Tüm kalkış şehirleri için şehir seçimi yapılmalı';
      }
    }
    
    if (!formData.nights?.trim()) newErrors.nights = 'Gece sayısı gerekli';
    if (!formData.maxParticipants || formData.maxParticipants <= 0) newErrors.maxParticipants = 'Maksimum katılımcı sayısı gerekli';
    ...
    if (formData.destinations.length === 0) {
      ...
      const emptyDestinations = formData.destinations.filter(dest => !dest.city?.trim());
      if (emptyDestinations.length > 0) {
        newErrors.destinations = 'Tüm destinasyonlar için şehir seçimi yapılmalı';
      }
    }
    
    // Konaklama tipi sadece günübirlik tur değilse zorunlu
    if (formData.tourType && formData.tourType !== 'Günübirlik Tur' && !formData.accommodationType?.trim()) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const filteredDestinations = formData.destinations
        ...
        .map(dest => ({ city: dest.city, description: dest.description || '' }));

      // Boş kalkış şehirlerini filtrele
      const filteredDepartureCities = formData.departureCity
      ...
      const formDataToSubmit = {
        ...
        tourDates: formData.tourDates.map(date => ({
          ...
          earlyBirdDeadline: date.earlyBirdDeadlineEnd && date.earlyBirdDeadlineEnd.trim() ? new Date(date.earlyBirdDeadlineEnd).toISOString() : null,
          lastMinuteStart: date.lastMinuteStartStart && date.lastMinuteStartStart.trim() ? new Date(date.lastMinuteStartStart).toISOString() : null,
          lastMinuteStartEnd: date.lastMinuteStartEnd && date.lastMinuteStartEnd.trim() ? new Date(date.lastMinuteStartEnd).toISOString() : null,
          notes: date.notes || '',
          status: date.status || 'ACTIVE',
          ageRanges: date.ageRanges ? date.ageRanges.filter(range => 
        ...
        pickupPoints: formData.pickupPoints
          .filter(point => point.city && point.city.trim() && point.location && point.location.trim() && point.time && point.time.trim())
          .map((point, index) => ({
      ...
      // isUpdateMode'a göre farklı endpoint ve method kullan
      const url = isUpdateMode ? `/api/partner/tours/${tourId}` : '/api/tours';
    ...
    if (formData.destinations.length > 0) {
      ...
      if (selectedCities.length > 0) {
        ...
        if (selectedCities.length === 1) {
          ...
          const cityRegions = selectedCities.map(city => getRegionByCity(city));
          const uniqueRegions = Array.from(new Set(cityRegions)).filter(region => region);
          ...
          if (uniqueRegions.length === 1) {
            ...
            const isNeighboring = regionPairs.some(pair => 
              (pair[0] === uniqueRegions[0] && pair[1] === uniqueRegions[1]) ||
            ...
            if (isNeighboring) {
              ...
              const region1Size = regionSizes[uniqueRegions[0] as keyof typeof regionSizes] || 0;
              const region2Size = regionSizes[uniqueRegions[1] as keyof typeof regionSizes] || 0;
              
              calculatedRegion = region1Size >= region2Size ? uniqueRegions[0] : uniqueRegions[1];
            } else {
              ...
              calculatedRegion = Object.entries(regionCounts)
                .sort(([,a], [,b]) => b - a)[0]?.[0] || '';
            }
          } else {
  ...
  const handleAddAgeRange = (tourDateIndex: number) => {
    ...
    let startAge: number | null = sortedRanges.length === 0 ? (parseInt(formData.ageRestriction) || 0) : null;
    if (sortedRanges.length > 0) {
      const lastRange = sortedRanges[sortedRanges.length - 1];
      // Son yaş aralığının bitiş yaşı girilmemişse yeni aralık eklenemez
      if (lastRange.maxAge === null || lastRange.maxAge === undefined || isNaN(Number(lastRange.maxAge))) {
      ...
      if (lastRange.maxAge <= lastRange.minAge) {
        alert('Mevcut yaş aralığının bitiş yaşı, başlangıç yaşından küçük veya eşit olamaz.');
        return;
      }
    ...
    for (let i = 0; i < sortedRanges.length; i++) {
      ...
      if (currentRange.maxAge !== null && startAge <= currentRange.maxAge) {
        alert(`Yeni yaş aralığı mevcut yaş aralığı (${currentRange.minAge}-${currentRange.maxAge}) ile çakışıyor. Önce mevcut aralıkları düzenleyin.`);
        return;
      }
    }
  ...
  const handleAgeRangeChange = (
    ...
    if (field === 'minAge' || field === 'maxAge') {
      ...
      if (value === 'fixed') {
        ageRange.value = String(parseFloat(newFormData.tourDates[tourDateIndex].price) || 0);
      }
    }
    setFormData(newFormData);
  };
    ...
    if (formData.tourDates.length > 0) {
      ...
      const updatedFirstDate: TourDate = {
        ...
        ageRanges: firstDate.ageRanges.map(range => ({
          ...range,
          value: range.pricingType === 'fixed' ? formData.price : (range.value || '0')
        }))
      };
  ...
  const availableLanguages = ['Türkçe', 'İngilizce', 'Almanca', 'Fransızca', 'Rusça', 'Arapça', 'İspanyolca', 'İtalyanca', 'Çince', 'Japonca'];

  const handleLanguageInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setNewLanguage(value);
    if (value.trim()) {
      const filtered = availableLanguages.filter(lang => lang.toLowerCase().includes(value.toLowerCase()));
  ...
  const handleAddLanguage = () => {
    if (newLanguage.trim() && availableLanguages.includes(newLanguage.trim()) && !formData.languages.includes(newLanguage.trim())) {
      ...
      {/* Temel Bilgiler */}
        {step === 'basic' && (
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
              <label htmlFor="departureCity" className="mb-2 block text-sm font-medium text-gray-900">
                ...
                <span className="ml-2 text-xs text-gray-500">(Birden fazla şehir seçebilirsiniz)</span>
              </label>
                  ...
                  <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                    <div className="flex-1">
                      ...
                      >
                        ...
                        {TURKEY_CITIES.filter(availableCity => 
                          !formData.departureCity.some((selectedCity, i) => i !== index && selectedCity === availableCity)
                        ).map(availableCity => (
            ...
            <div>
              ...
              <div className="space-y-3">
                  ...
                  <div key={index} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                    <div className="flex-1">
                      <select
                        ...
                        onChange={(e) => handleDestinationsChange(index, 'city', e.target.value)}
                        className="block w-full rounded-lg border border-gray-300 px-4 py-2 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-900"
                      >
                        <option value="">Şehir seçiniz</option>
                        {TURKEY_CITIES.filter(city => 
                          !formData.departureCity.includes(city) && 
                          !formData.destinations.some((dest, i) => i !== index && dest.city === city)
                        ).map(city => (
                ...
                <button
                  ...
                  onClick={() => {
                    setFormData(prev => ({
                      ...prev,
                      destinations: [...prev.destinations, { city: '', description: '' }]
                    }));
                  }}
                  className="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-lg text-gray-700 bg-white hover:bg-gray-50"
                >
              ...
              {errors.destinations && <p className="mt-2 text-sm text-red-600">{errors.destinations}</p>}
            </div>
          </div>
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-gray-900">
              Bölge
              <span className="ml-2 text-xs text-gray-500">(Otomatik hesaplanır)</span>
            </label>
            <div className="relative">
              <input
                type="text"
              ...
              />
              {formData.destinations.length > 0 && formData.destinations.some(dest => dest.city) && (
                  ...
                  {(() => {
                    ...
                    if (selectedCities.length === 1) {
                      ...
                      if (uniqueRegions.length === 1) {
                        ...
                        const regionPairs = [
                          ['Marmara', 'Ege'], ['Ege', 'Akdeniz'], ['Akdeniz', 'İç Anadolu'],
                          ['İç Anadolu', 'Karadeniz'], ['İç Anadolu', 'Doğu Anadolu'],
                          ['Doğu Anadolu', 'Güneydoğu Anadolu'], ['Karadeniz', 'Doğu Anadolu']
                        ];
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  ...
                  onChange={(val) => {
                    setFormData(prev => {
                      ...
                      if (updatedTourDates.length > 0) {
                        ...
                        const endDate = new Date(startDate);
                        endDate.setDate(startDate.getDate() + (parseInt(prev.nights) || 0));
            ...
            <div>
              <label className="block text-sm font-medium text-gray-900">Bitiş Tarihi</label>
          ...
          <div className="mb-6">
            <label className="mb-2 block text-sm font-medium text-gray-900">
              ...
              <span className="ml-2 text-xs text-gray-500">(En az 3, en fazla 10 görsel)</span>
            </label>
            <input
              type="file"
              accept="image/*"
            ...
            />
            {errors.galleryImages && <p className="mt-2 text-sm text-red-600">{errors.galleryImages}</p>}
              ...
              {formData.galleryImages.map((img, idx) => (
                <div key={idx} className="relative group border rounded-lg p-2 bg-white">
                  ...
                  <input
                    ...
                    onChange={e => handleGalleryImageDescriptionChange(idx, e.target.value)}
                  ...
                  <div className="flex justify-between mt-1">
                    <button type="button" disabled={idx === 0} onClick={() => handleReorderGalleryImages(idx, idx - 1)} className="text-xs px-2 py-1 bg-gray-100 rounded disabled:opacity-50">↑</button>
                    <button type="button" disabled={idx === formData.galleryImages.length - 1} onClick={() => handleReorderGalleryImages(idx, idx + 1)} className="text-xs px-2 py-1 bg-gray-100 rounded disabled:opacity-50">↓</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            <div>
              <label className="mb-2 block text-sm font-medium text-gray-900">Diller <span className="text-red-500">*</span></label>
              <div className="flex gap-2">
                ...
                />
                <button type="button" onClick={handleAddLanguage} className="px-4 py-3 rounded-lg bg-indigo-600 text-white">Ekle</button>
              </div>
              {suggestions.length > 0 && (
                <ul className="mt-2 border border-gray-300 rounded-lg bg-white shadow-sm">
                  {suggestions.map((suggestion, idx) => (
                    <li key={idx} className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-gray-900" onClick={() => handleSuggestionClick(suggestion)}>{suggestion}</li>
                  ))}
                </ul>
              )}
              <div className="mt-2 flex flex-wrap gap-2">
                {formData.languages.map((lang, idx) => (
                  <span key={idx} className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm bg-gray-100 text-gray-800">
                    {lang}
                    <button type="button" onClick={() => handleRemoveLanguage(idx)} className="text-gray-500 hover:text-red-500"><XMarkIcon className="h-4 w-4" /></button>
                  </span>
                ))}
              </div>
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-gray-900">Etiketler</label>
              <div className="mt-2">
            ...
            <h2 className="text-lg font-medium text-gray-900">Detaylar</h2>
              ...
              <div className="mt-2 space-y-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      if (newExclude.trim() && !formData.excludes.includes(newExclude.trim())) {
                        setFormData(prev => ({
                          ...prev,
                          excludes: [...prev.excludes, newExclude.trim()]
                          }));
              ...
              <div>
                ...
                {formData.itinerary.map((day, index) => (
                  <div key={index} className="flex flex-col gap-4 p-4 bg-white rounded-lg border border-gray-200 shadow-md mb-4">
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        value={day.title}
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-900 mb-2">Gün Açıklaması</label>
                      <div data-color-mode="light" className="border border-gray-300 rounded-lg overflow-hidden">
                        <MDEditor
                          ...
                          hideToolbar={false}
                          textareaProps={{
                            placeholder: "Bu günün detaylarını açıklayın... Markdown formatında yazabilirsiniz.",
                            style: { fontSize: '14px' }
                          }}
                        />
                      </div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-900 mb-1">Gün Fotoğrafları</label>
                      <input
                        type="file"
                        accept="image/*"
                        multiple
                        onChange={e => handleItineraryImageAdd(index, e.target.files)}
                        className="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none"
                      />
                        ...
                        <div className="mt-2 flex flex-wrap gap-3">
                          {day.images.map((img, imgIdx) => (
                            <div key={imgIdx} className="relative group w-24 h-24">
                              <img src={img.url} alt="itinerary-img" className="object-cover w-full h-full rounded-lg border" />
                              <button
                                type="button"
                                onClick={() => handleItineraryImageRemove(index, imgIdx)}
        ...
        <div className="bg-white shadow rounded-lg p-6">
          ...
          <div className="space-y-6">
            {formData.tourDates.map((tourDate, index) => (
              <div key={index} className="bg-white rounded-lg border border-gray-200 mb-4 overflow-hidden">
                <div 
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-500 mt-1">
                        ...
                        {tourDate.availableSeats && ` • ${tourDate.availableSeats} kişilik kontenjan`}
                        {tourDate.soldSeats && parseInt(tourDate.soldSeats) > 0 && ` • ${tourDate.soldSeats} kişi satıldı`}
                      </div>
                    </div>
                  </div>
                  {index > 0 && (
                ...
                <div 
                  className={`overflow-hidden transition-all duration-200 ${
                    tourDate.isExpanded ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'
                  }`}
                >
                  <div className="p-6 space-y-6">
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Başlangıç Tarihi</label>
                        <div className="relative mt-1">
                          <DatePicker
                            label=""
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Kontenjan</label>
                        <div className="mt-1">
                          <input
                            type="number"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Satılan Koltuk</label>
                        <div className="mt-1">
                          <input
                            type="number"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Min. Katılımcı</label>
                        <div className="mt-1">
                          <input
                            type="number"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Max. Katılımcı</label>
                        <div className="mt-1">
                          <input
                            type="number"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Erken Rezervasyon İndirimi (%)</label>
                        <div className="mt-1">
                          <input
                            type="number"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Son Dakika İndirimi (%)</label>
                        <div className="mt-1">
                          <input
                            type="number"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Erken Rez. Başlangıç</label>
                        <div className="relative mt-1">
                          <DatePicker
                            label=""
                            value={tourDate.earlyBirdDeadlineStart}
                            onChange={(val) => handleTourDateChange(index, 'earlyBirdDeadlineStart', val)}
                            placeholder="gg.aa.yyyy"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Erken Rez. Bitiş</label>
                        <div className="relative mt-1">
                          <DatePicker
                            label=""
                            value={tourDate.earlyBirdDeadlineEnd}
                            onChange={(val) => handleTourDateChange(index, 'earlyBirdDeadlineEnd', val)}
                            placeholder="gg.aa.yyyy"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Son Dakika Başlangıç</label>
                        <div className="relative mt-1">
                          <DatePicker
                            label=""
                            value={tourDate.lastMinuteStartStart}
                            onChange={(val) => handleTourDateChange(index, 'lastMinuteStartStart', val)}
                            placeholder="gg.aa.yyyy"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Son Dakika Bitiş</label>
                        <div className="relative mt-1">
                          <DatePicker
                            label=""
                            value={tourDate.lastMinuteStartEnd}
                            onChange={(val) => handleTourDateChange(index, 'lastMinuteStartEnd', val)}
                            placeholder="gg.aa.yyyy"
                      ...
                      <div>
                        ...
                        <div className="mt-1">
                          <select
                            ...
                            onChange={(e) => handleTourDateChange(index, 'status', e.target.value)}
                            className="block w-full rounded-lg border border-gray-300 px-4 py-3 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-900"
                          >
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-900">Notlar</label>
                        <div className="mt-1">
                          <textarea
                    ...
                    <div className="col-span-2">
                      ...
                      {tourDate.ageRanges?.map((ageRange, ageIndex) => (
                        <div key={ageIndex} className="flex gap-3 items-start mb-3 bg-white p-4 rounded-lg border border-neutral-200 shadow-sm">
                          <div className="flex-1">
                            <label className="block text-xs font-medium text-neutral-600 mb-1">Başlangıç Yaşı</label>
                            <input
                              type="text"
                              inputMode="numeric"
                              pattern="\d*"
                              ...
                              onChange={(e) => {
                                if (ageIndex === 0) {
                                  ...
                                  if (value === '' || /^\d+$/.test(value)) {
                                    handleAgeRangeChange(index, ageIndex, 'minAge', value ? Number(value) : 0);
                                  }
                                }
                              }}
                            />
                          ...
                          <div className="flex-1">
                            <label className="block text-xs font-medium text-neutral-600 mb-1">Bitiş Yaşı</label>
                            <input
                              type="text"
                              inputMode="numeric"
                              pattern="\d*"
                              ...
                              onChange={(e) => {
                                const value = e.target.value.replace(/^0+/, '');
                                if (value === '' || /^\d+$/.test(value)) {
                                  handleAgeRangeChange(index, ageIndex, 'maxAge', value ? Number(value) : null);
                                }
                              }}
                          ...
                          <div className="flex-1">
                            <label className="block text-xs font-medium text-neutral-600 mb-1">Fiyatlandırma Tipi</label>
                            <select
                              value={ageRange.pricingType}
                              onChange={(e) => handleAgeRangeChange(index, ageIndex, 'pricingType', e.target.value)}
                          ...
                          <div className="flex-1">
                            ...
                            <input
                              ...
                              onChange={(e) => handleAgeRangeChange(index, ageIndex, 'value', e.target.value)}
                              className="w-full px-3 py-2 border border-neutral-300 rounded-md text-neutral-800 placeholder-neutral-400 focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
          ...
          disabled={isSubmitting}
                ...
                >
                  {isSubmitting ? (
                    ...
                    isUpdateMode ? 'Değişiklikleri Kaydet' : 'Turu Oluştur'
                  )}
        </button>
              </>
            )}
