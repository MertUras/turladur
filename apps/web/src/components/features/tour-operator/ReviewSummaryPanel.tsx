...
export default function ReviewSummaryPanel({
  ...
  const { averageRating, reviewCount, starDistribution, categoryAggregates } =
    aggregates;
  const maxDistributionCount = Math.max(
    ...STAR_LEVELS.map((star) => starDistribution[star]),
    1,
  );
          ...
          {STAR_LEVELS.map((star) => {
            ...
            return (
              <div key={star} className="flex items-center gap-2 text-xs">
                <span className="w-7 text-neutral-600 font-medium shrink-0">
                  ★{star}
                </span>
                ...
                <span className="w-8 text-right text-neutral-500 tabular-nums shrink-0">
                  {count}
                </span>
              </div>
            );
