...
import { Bus, ClipboardList, Home, LogOut, Menu, X } from 'lucide-react';

import { BrandLogo } from '@/components/brand/brand-logo';
import { useAuth } from '@/providers/auth-provider';
import { OTOBUS_LOGIN, OTOBUS_ROUTES } from '@/lib/panel-routes';
import { cn } from '@/lib/utils';

const NAV = [
  { name: 'Genel Bakış', href: OTOBUS_ROUTES.dashboard, icon: Home },
  { name: 'Araçlar', href: OTOBUS_ROUTES.vehicles, icon: Bus },
  { name: 'Atamalar', href: OTOBUS_ROUTES.assignments, icon: ClipboardList },
];
...
/** Bus company panel shell — AgencyShell layout parity (P0-B). */
export function BusShell({ children }: { children: ReactNode }) {
  ...
  const name = user ? displayName(user) : 'Otobüs';

  useEffect(() => {
    if (isBootstrapping) return;
    if (!accessToken || !user) {
      setGate('denied');
      router.replace(OTOBUS_LOGIN);
      return;
    }
  ...
  if (gate !== 'allowed' || !isAuthenticated) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-3 px-4 text-center text-neutral-600">
        <p>Otobüs paneli için giriş gerekli…</p>
        <Link
          href={OTOBUS_LOGIN}
        ...
        <div className="mb-6 px-4 pt-4">
          <div className="flex items-center rounded-lg bg-blue-50 p-3">
            ...
            <div className="ml-3 min-w-0">
              ...
              <p className="text-xs text-gray-500">Otobüs firması</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex h-16 items-center justify-between px-4 md:px-6">
            ...
            <h1 className="text-lg font-semibold text-gray-900">
              {NAV.find(
                (item) =>
                  isActive(item.href) ||
                  (item.href === OTOBUS_ROUTES.vehicles &&
                    pathname.startsWith('/otobus/araclar')),
              )?.name ?? 'Genel Bakış'}
            </h1>
            <button
              type="button"
              onClick={() => {
                void logout().then(() => router.replace(OTOBUS_LOGIN));
              }}
